use crate::ir::{Literal, Source, Statement};
...
pub fn parse_program(input: &str) -> Result<Vec<Statement>, anyhow::Error> {
    ...
    for line in input.lines() {
        ...
        if let Some(eq_pos) = line.find('=') {
            ...
            && tokens[0].to_uppercase() == "PUT"
            && tokens[1].to_uppercase() == "SKIP"
            && tokens[2].to_uppercase() == "LIST"
        {
            // PUT SKIP LIST(...)
            let start = line
                .find('(')
                .ok_or_else(|| anyhow::anyhow!("Missing '(' in PUT"))?;
            let end = line
                .rfind(')')
                .ok_or_else(|| anyhow::anyhow!("Missing ')' in PUT"))?;
