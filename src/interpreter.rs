...
impl Interpreter {
    ...
    fn evaluate_expression(&self, expr: &crate::ir::Expression) -> i64 {
        match expr {
            crate::ir::Expression::Literal(v) => match v {
                crate::ir::Literal::Int(i) => *i,
                _ => 0,
            },

            crate::ir::Expression::Variable(name) => *self.vars.get(name).unwrap_or(&0),
    ...
    fn execute_statement(&mut self, stmt: &crate::ir::Statement) {
        match stmt {
            ...
            crate::ir::Statement::PerformVarying {
                ...
                let mut current = self.evaluate_expression(from);

                let step = self.evaluate_expression(by);

                self.vars.insert(variable.clone(), current);

                loop {
                    let left = self.eval_condition_value(&until.left);

                    let right = self.eval_condition_value(&until.right);
