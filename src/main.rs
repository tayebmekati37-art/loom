...
fn parse_legacy(input: &str, lang: &str) -> anyhow::Result<Vec<ir::Statement>> {
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Translate { input, lang, target } => {
            let statements = parse_legacy(&input, &lang)?;
        ...
        Commands::Validate { input, lang, inputs, record, test_file } => {
            ...
            let test_cases = if record {
                ...
                if std::path::Path::new(&test_path).exists() {
                    ...
                    let json_cases: Vec<serde_json::Map<String, serde_json::Value>> =
                    ...
                    for map in json_cases {
                        ...
                        for (k, v) in map {
                            let num = v.as_i64()
                                .ok_or_else(|| anyhow::anyhow!("Invalid test value for key {}", k))?;
                            hm.insert(k, num);
                        }
                        cases.push(hm);
                    }
        ...
        Commands::Migrate { legacy_file, modern_file, target } => {
            let legacy_statements = parse_legacy(&legacy_file, "simple")?;

            let legacy_func = ir::Function {
                name: "legacy_func".to_string(),
                body: legacy_statements,
            };
            ...
            if let Some(modern_path) = modern_file {
                let modern_statements = parse_legacy(&modern_path, "simple")?;

                let modern_func = ir::Function {
                    name: "modern_func".to_string(),
                    body: modern_statements,
                };
...
fn generate_test_cases(func: &ir::Function) -> anyhow::Result<Vec<HashMap<String, i64>>> {
    ...
    if vars.is_empty() {
        anyhow::bail!("No variables found to generate test cases");
    }
