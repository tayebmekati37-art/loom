...
fn parse_block(lines: &Vec<&str>, i: &mut usize, terminators: &[&str]) -> Result<Vec<Statement>> {
    let mut statements = Vec::new();
    ...
    while *i < lines.len() {
        ...
        if upper.starts_with("IF ") {
            ...
            *i += 1;
            ...
            let then_branch = parse_block(lines, i, &["ELSE", "END-IF"])?;

            let mut else_branch = None;

            if *i < lines.len() && lines[*i].trim().eq_ignore_ascii_case("ELSE") {
                *i += 1;

                else_branch = Some(parse_block(lines, i, &["END-IF"])?);
            }

            if *i < lines.len() && lines[*i].trim().eq_ignore_ascii_case("END-IF") {
                *i += 1;
            }
        ...
        if upper.starts_with("PERFORM UNTIL ") {
            ...
            *i += 1;
            ...
            let body = parse_block(lines, i, &["END-PERFORM"])?;

            if *i < lines.len() && lines[*i].trim().eq_ignore_ascii_case("END-PERFORM") {
                *i += 1;
            }

            statements.push(Statement::PerformUntil { condition, body });
